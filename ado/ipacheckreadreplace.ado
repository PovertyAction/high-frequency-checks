	...
	*/
	...
	qui {
		* check syntax
		cap isid `id'
		if _rc == 459 {
			noi disp as err "variable `id' does not uniquely identify the observations"
			ex 459
		}
		...
		* if extension is valid, import data
		if "`ext'" == ".xlsx" | "`ext'" == ".xls" {
			...
			di as err "file type `ext' not allowed. Use .xlsx .xls or .csv file"
			ex 609 
		}
		...
		* Save all values in locals
		loc rep_count `=_N'
		if `rep_count' > 0 {
			foreach var of varlist `id' `variable' `value' `newvalue' `action' {
			...
			* display header
			noi disp "Making `rep_count' changes to Dataset ... "
			
			if "`logusing'" ~= "" putexcel set "`logusing'", sh("log") replace
			if "`logusing'" ~= "" & `c(version)' >= 14.0 {
				putexcel A1:F1 = ("REPLACEMENTS LOG"), hcenter merge bold font(calibri, 12) border(bottom, double)
				putexcel A2 = ("`id'") 		///
					 B2 = ("`variable'") 	///
					 C2 = ("`value'") 		///
					 D2 = ("`newvalue'")	///
			...
			* Output status to excel
			if "`logusing'" ~= "" {
				...
				putexcel A`row' = ("``id'_`i''") ///
					...
					D`row' = ("``newvalue'_`i''") ///
					E`row' = ("``action'_`i''")	///
					F`row' = ("`status'"), font(calibri, 11, `color')
