   ...
   *command does not exist, show which 
   if _rc == 111 {
     *Prepare installation instructions
     ...
     if "`command'" == "bcstats" 		local install_code `"{inp: net install bcstats, replace from("https://raw.githubusercontent.com/PovertyAction/bcstats/master/ado")}"' //This code cannot be made an active link due to the : after https
